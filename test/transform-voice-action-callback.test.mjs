...
import { responseTransformer } from '../src/transformers/voice-action-callback.mjs'
const { expect } = chai
...
describe('transform voice_action callback (recording etc)', () => {
  describe('response', () => {
    ...
    it('converts wav to link', () =>
      expectDeepEqual({
        actual: responseTransformer('http://BASE_URL')({ wav: 'http://wav' }),
        expected: { _links: { wav: { href: 'http://wav' } } },
      }))
    it('converts callid to calls/:id link', () =>
      expectDeepEqual({
        actual: responseTransformer('http://BASE_URL')({ callid: 'qwerty' }),
        expected: {
          _links: { call: { href: 'http://BASE_URL/v2/me/calls/qwerty' } },
        },
      }))
    it('converts both callid and wav if available', () =>
      expectDeepEqual({
